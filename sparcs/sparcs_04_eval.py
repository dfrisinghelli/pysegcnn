...
# local modules
from sparcs.sparcs_00_config import state_path
from sparcs.sparcs_02_dataset import state_file, trainer

if __name__ == '__main__':

    # predict each batch in the validation set
    cm, accuracy = trainer.predict(state_path, state_file, confusion=True)
    ...
    samples = np.random.randint(len(trainer.valid_ds), size=n)
