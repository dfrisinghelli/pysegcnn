...
INFERENCE_NAMES = ['x', 'y_true', 'y_pred']
...
class NetworkInference(BaseConfig):
    ...
    def evaluate(self):
        ...
        for state in self.state_files:
            ...
            y_true = np.asarray([v[INFERENCE_NAMES[1]].flatten() for _, v
                                 in output.items()]).flatten()
            y_pred = np.asarray([v[INFERENCE_NAMES[2]].flatten() for _, v
                                 in output.items()]).flatten()
        ...
        if self.aggregate:
            ...
            base_name = self.state_files[0]
            fold_number = re.search('f[0-9]', base_name.name)[0]
            ...
            # add aggregated classification report
